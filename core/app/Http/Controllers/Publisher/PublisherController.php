...
class PublisherController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $transaction = EarningLogs::whereYear('created_at', '>=', Carbon::now()->subYear())->selectRaw("SUM(CASE WHEN publisher_id = $user THEN amount END) as amount")->selectRaw("DATE_FORMAT(created_at,'%M') as months")->orderBy('created_at')->groupBy(DB::Raw("MONTH(created_at)"))->get();

        $w = Withdrawal::whereYear('created_at', '>=', Carbon::now()->subYear())->where('status', 1)->where('user_id', $user)->orderBy('created_at')->selectRaw("SUM(CASE WHEN user_id = $user THEN amount END) as amount")->selectRaw("DATE_FORMAT(created_at,'%M') as months")->groupBy(DB::Raw("MONTH(created_at)"))->get();

        $w->map(
            function ($item) use ($wdata){
    ...
    public function domainVerification()
    {
        ...
        $domainVerifications = DomainVerifcation::where('publisher_id', auth()->guard('publisher')->user()->id)->orderBy('domain_name', 'asc')->paginate(50);
    ...
    public function domainVerify(Request $request)
    {
        ...
        if(!empty($url == $domainVerifcation))
        {
            ...
            $domain->keywords     = implode(', ', $new_key);
        ...
        else
        {
            ...
            $domainVerify->keywords     = implode(', ', $new_key);
    ...
    public function domainCheck($tracker)
    {
        ...
        if(Auth::guard('publisher')->user()->id == $domain->publisher_id)
        {
            ...
            if($domain->verify_code == $verification)
            {
                ...
                $domain->save();
                $general->domain_approval == 1
                    ? $notify[] = [
                    'info',
                    'Your domain has been submitted for approval',
                ]
                    : $notify[] = [
                    'success',
        ...
        $notify[] = [
            'error',
            'We couldn\'t verify your domain. Please try again',
        ];
    ...
    public function dateSearch(Request $request)
    {
        ...
        else
        {
            ...
            $logs       = EarningLogs::wherePublisherId(auth()->guard('publisher')->id())->whereBetween(
                'date', [
                          $firstDate,
                          $secondDate,
                      ]
    ...
    public function disable2fa(Request $request)
    {
        ...
        if($oneCode == $userCode)
        {
            ...
            $notify[] = [
                'success',
                'Two Factor Authenticator Disable Successfully',
            ];
    ...
    public function withdrawStore(Request $request)
    {
        ...
        if($request->amount < $method->min_limit)
        {
            $notify[] = [
                'error',
                'Your Requested Amount is Smaller Than Minimum Amount.',
            ];
        ...
        if($request->amount > $method->max_limit)
        {
            $notify[] = [
                'error',
                'Your Requested Amount is Larger Than Maximum Amount.',
            ];
        ...
        if($request->amount > $user->earnings)
        {
            $notify[] = [
                'error',
                'Your do not have Sufficient Balance For Withdraw.',
            ];
    ...
    public function withdrawSubmit(Request $request)
    {
                        ...
                        if($inVal->type == 'file')
                        {
                            if($request->hasFile($inKey))
                            {
                                ...
                                catch(\Exception $exp)
                                {
                                    $notify[] = [
                                        'error',
                                        'Could not upload your ' . $request[$inKey],
                                    ];
        ...
        $notify[] = [
            'success',
            'Withdraw Request Successfully Send',
        ];
    ...
    public function adReportSearch(Request $request)
    {
        ...
        if($request->date)
        {
            ...
            else
            {
                $firstDate  = Carbon::create($date[0])->format('Y-m-d');
                $secondDate = Carbon::create($date[1])->format('Y-m-d');
                $reports    = PublisherAd::wherePublisherId(auth()->guard('publisher')->id())->whereBetween(
                    'date', [
                              $firstDate,
                              $secondDate,
                          ]
        ...
        {
            $reports = PublisherAd::wherePublisherId(auth()->guard('publisher')->id())->whereHas(
                'advertise', function ($q) use ($request){
